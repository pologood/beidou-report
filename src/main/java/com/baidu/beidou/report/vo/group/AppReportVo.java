...
public class AppReportVo extends AbstractReportVo {
	...
	/** 明细 */
	protected List<AppViewItem> details;

	/** 汇总信息 */
	protected AppReportSumData summary;
	...
	public AppReportVo(String level) {
		super(level);
	}
	...
	public List<String[]>  getCsvReportDetail() {
		...
		for (AppViewItem item : details ){
			int index = 0;
			detailStringArray = new String[headers.length];
			detailStringArray[index] = item.getAppName();
			detailStringArray[++index] = String.valueOf(item.getAppId());
	...
	public String[] getCsvReportSummary() {
		...
		summaryStringArray[++index] = "";//所属一级行业
		summaryStringArray[++index] = "";//所属二级行业
	...
	public AppReportSumData getSummary() {
		return summary;
	}

	public void setSummary(AppReportSumData summary) {
		this.summary = summary;
	}
	
	public List<AppViewItem> getDetails() {
		return details;
	}

	public void setDetails(List<AppViewItem> details) {
