...
public class SiteReportVo extends AbstractReportVo {
	
	private List<SiteViewItem> details;
	private SiteSumData sumData;
	
	protected boolean showTransData;//是否显示转化数据列
	protected boolean transDataValid;//如果显示转化数据列，那么非valid的情况下应该显示-
	
	public List<SiteViewItem> getDetails() {
		return details;
	}

	public void setDetails(List<SiteViewItem> details) {
		this.details = details;
	}

	public SiteReportVo(){}
	
	public SiteReportVo(String level){
		super(level);
	}
	...
	public List<String[]> getCsvReportDetail() {
		...
		for (SiteViewItem item : details ){
			int index = 0;
			detailStringArray = new String[headers.length];
			detailStringArray[index] = item.getSiteUrl();
			...
			//modify by dongying since cpweb-611-1 拆分一二级行业信息
			detailStringArray[++index] = item.getFirstTrade();
			if(item.getSecondTrade() != null){
	...
	public String[] getCsvReportSummary() {
		...
		summaryStringArray[++index] = "";//一级行业
		summaryStringArray[++index] = "";//二级行业
		...
		summaryStringArray[++index] = -1 == sumData.getSrchuv() ? "-" : String.valueOf(sumData.getSrchuv());//展现独立访客
		summaryStringArray[++index] = "";//展现频次
		summaryStringArray[++index] = "" + sumData.getClks();
		summaryStringArray[++index] = -1 == sumData.getClkuv() ? "-" : String.valueOf(sumData.getClkuv());//点击独立访客
		...
		summaryStringArray[++index] = -1 == sumData.getCost() ? "-" : String.valueOf(sumData.getCost());//消费
	...
	public void setSumData(SiteSumData sumData) {
		if(sumData == null){
			sumData = new SiteSumData();
