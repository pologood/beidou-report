...
import com.baidu.beidou.period.stat.AccountBean;
...
public class UserAccountCustomReportAction extends BeidouReportActionSupport {
	private static final long serialVersionUID = 1116584041553970033L;
	private static Log log = LogFactory
			.getLog(UserAccountCustomReportAction.class);

	private static final String downloadTitle = "账户效果报告";

	@Resource
	private PeriodReportAPI pdApi;

	// private UserCustomReportFacade userCustomReportFacade = null;
	//
	// public UserCustomReportFacade getUserCustomReportFacade() {
	// return userCustomReportFacade;
	// }
	//
	// public void setUserCustomReportFacade(
	// UserCustomReportFacade userCustomReportFacade) {
	// this.userCustomReportFacade = userCustomReportFacade;
	// }
...
//		}
		...
		report.setReportType(ReportConstants.ReportType.ACCOUT_REPORT);
		Result<ReportResponse<AccountBean>> result = pdApi.getResponse(report);
		if (!result.isSuccess()) {
			return ERROR;
		}
		ReportResponse<AccountBean> response = result.getResult();
		List<AccountBean> list = response.getDataList();
...
//		}
		...
		try {
			...
			fileName = user.getUsername() + "-"
					+ this.getText("customreport.name.account");
