 ...
 */
public class BeidouCustomizeParametersInterceptor extends MethodFilterInterceptor {

	private static final long serialVersionUID = -6726447823997360541L;

	private static final Logger LOG = LoggerFactory.getLogger(BeidouCustomizeParametersInterceptor.class);

	//public static final String ACCEPTED_PARAM_NAMES = "\\w+((\\.\\w+)|(\\[\\d+\\])|(\\(\\d+\\))|(\\['\\w+'\\])|(\\('\\w+'\\)))*";
	public static final String ACCEPTED_PARAM_NAMES = "[[\\p{Graph}\\s]&&[^,#:=]]*";
    ...
    public static void main(String[] args) {
    	String input = "ahttp://baidu.comsdf/";
    	//String input = "baidu.com";
    	
    	String PARAM_NAMES = ".*[\\(\\)#@'].*";

    	//String PARAM_NAMES = "\\w+http://((\\[\\d+\\])|(\\(\\d+\\))|(\\['\\w+'\\])|(\\('\\w+'\\)))*";
    	Pattern acceptedPattern = Pattern.compile(PARAM_NAMES);
    	//Pattern acceptedPattern = Pattern.compile(ACCEPTED_PARAM_NAMES);
    	Matcher matcher = acceptedPattern.matcher(input);
